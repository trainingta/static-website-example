...
pipeline {
    environment {
        ...
        APP_EXPOSED_PORT = "80"
        IMAGE_TAG = "latest"
        STAGING = "chocoapp2022-staging"
        PRODUCTION = "chocoapp2022-prod"
          ...
          steps {
            script {
              sh '''
                  ...
                  docker run --net=host --name $IMAGE_NAME -d -p $APP_EXPOSED_PORT:$APP_CONTAINER_PORT -e PORT=$APP_CONTAINER_PORT ${DOCKERHUB_ID}/$IMAGE_NAME:$IMAGE_TAG
                  sleep 5
              '''
             }
          }
